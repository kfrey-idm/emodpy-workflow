...
from emodpy_workflow.lib.analysis.condition import Condition
...
class DataFrameWrapper:
    ...
    def verify_required_items(self, needed, available=None):
        ...
        if len(missing_items) > 0:
            raise self.MissingRequiredData('Missing required item(s): %s for operation' %
    ...
    def from_directory(cls, directory, file_type=None, stratifiers=None):
        ...
        if len(individual_objects) > 0:
            combined_df = pd.concat([obj._dataframe for obj in individual_objects], sort=True)
        else:
            combined_df = pd.DataFrame()
        stratifiers = set().union(*[obj.stratifiers for obj in individual_objects])
